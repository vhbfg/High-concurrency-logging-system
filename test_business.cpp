...
// 一、基础测试1：三分类日志基本写入。APP/OPERATION/ERROR 各写一条，验证三个加密日志文件均生成。
bool test_basic_log()
{
...
// 基础测试2：日志级别过滤。初始化级别设为 WARN，写入 INFO/DEBUG（应过滤）+ ERROR（应通过），验证入队数变化精确匹配。
...
// 基础测试3：日志文件自动切分。写入 MAX_LINES_PER_FILE + 50 = 1050 条，验证第二个分段文件生成。
...
// 基础测试4：队列满载测试。初始化队列容量为4，写入6条日志，验证入队和丢弃数量准确
...
// 基础测试5：XOR 加解密正确性。写入带标记文本，读密文解密，验证明文包含原始标记。
