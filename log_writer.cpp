...
void LogWriter::init() {
    app_.dir = root_ + "/application";
    app_.prefix = "application";
    op_.dir = root_ + "/operation";
    op_.prefix = "operation";
    err_.dir = root_ + "/error";
    err_.prefix = "error";
...
void LogWriter::write(const LogRecord& rec) {
    ...
    std::string enc = xor_encrypt_to_hex(rec.text);
    st.ofs << enc << '\n';

    if (++st.lines >= (int)MAX_LINES_PER_FILE) {
        rotate(st);
    }
}
