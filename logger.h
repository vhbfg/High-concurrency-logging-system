...
public:
    ...
    void close();
    void add(LogLevel,const char* fmt,...);
...
bool log_init(const char* dir,LogLevel min_level=LogLevel::DEBUG,QueueKind kind=QueueKind::Mutex);
void log_app(LogLevel level,const char* fmt,...);
void log_operator(LogLevel,const char* fmt,...);
